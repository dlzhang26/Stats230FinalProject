...
**Fig. 5. Pre-Injury Play Time With the Outliers.** After conducting bivariate analysis for Post-Injury Play Time vs. Pre-Injury Play Time, we noticed two unusal points: one at Pre-Injury Play Time = 0 and Post-Injury Play Time > 80, and another at Pre-Injury Play Time at around 83 and Post-Injury Play Time <25. 

```{r warning = FALSE, echo = FALSE, fig.height = 3, fig.width = 4}
plot(PostIPTvsPreIPT, which = 5)
```

**Fig. 5a. evaluating Pre-Injury Play Time Unusual Points** Observation 30 (Observation 37 in the actual data set) has high leverage () and standardized residuals (over 4). NAME DROP AND EXPLAIN POSSIBLE REASONING. 69 has high negative standardized residuals but moderate leverage ###, so we decided not to remove. 
...
```
**Fig. 7.** There is little to no relationship between a player's post-injury play time and the age at which they sustained the concussion (r = 0.0414). There is also no relationship between a player's post-injury play time and the week at which they sustained the concussion (r = 0.00207).
