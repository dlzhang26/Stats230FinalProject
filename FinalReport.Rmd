...
**Fig. 5. Pre-Injury Play Time With the Outliers.** After conducting bivariate analysis for Post-Injury Play Time vs. Pre-Injury Play Time, we noticed two unusual points: one at Pre-Injury Play Time = 0 and Post-Injury Play Time > 80, and another at Pre-Injury Play Time at around 83 and Post-Injury Play Time <25. We will evaluate the most extreme outlier and see if conditions improve by only removing one point to preserve numbers. 

```{r warning = FALSE, echo = FALSE, fig.height = 3, fig.width = 4}
plot(PostIPTvsPreIPT, which = 5)
```

**Fig. 5a. evaluating Pre-Injury Play Time Unusual Points** Observation 30 (Observation 37 in the actual data set)is the most unusual point in our dataset. This player is Chris Prosinski, and he is considered an extreme outlier according to his standardized residuals of over 4 (threshold is 3) and high leverage of over 3.5 (high leverage threshold at 6/203 = 0.0296). One possible explanation was that Prosinski's injury was not severe enough keep him out of the field, but he could have been benched for the remaining two games of the regular season, resulting in an average Post-Injury play time of 0. We will remove this player and see if conditions improve.
...
```
**Fig. 6. Pre-Injury Play Time Without the Outlier.** After removing the outlier, the residuals and QQ plot improved. There is a strong, positive, linear relationship between Post-Injury Play Time and Pre-Injury Play Time (r = 0.658). In other words, the more pre-injury play time a player had, the more post-injury play time a player had.  
...
```
**Fig. 7.** There is little to no relationship between a player's post-injury play time and the age at which they sustained the concussion (r = 0.0414). There is also little relationship between a player's post-injury play time and the week at which they sustained the concussion (r = 0.00207).
