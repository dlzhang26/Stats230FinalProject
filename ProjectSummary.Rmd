...
gf_bar(~PreseasonInjury,data = studydata ) 
...
finalData1 <- select(finalData, "PostIPT" , "amtGamesMissed", "PreIPT", "TotalSnaps", "WeekOfInjury", "AgeAtConcussion", "PreseasonInjury", "Position", "Conference", "Team", "OppTeamC", "OnWinningTeam", "PositionC")
```
...
##qualitative
m6 <- gf_bar(~ PositionC, data = finalData1, fill = c("lavender", "skyblue"))
...
###TESTING IF CONFERENCE MATTERED
```{r}
tally(~Conference, data = finalData1 )
gf_boxplot(PostIPT ~ Conference, data = finalData1)

gf_dens( ~ PostIPT, color = ~Conference, data = finalData1)
favstats(PostIPT ~ Conference, data = finalData1)

conferenceMod <- lm(PostIPT ~ Conference, data = finalData1)
anova(conferenceMod)




tally(~OppTeamC, data = finalData1 )
gf_boxplot(PostIPT ~ OppTeamC, data = finalData1)

gf_dens( ~ PostIPT, color = ~OppTeamC, data = finalData1)
favstats(PostIPT ~ OppTeamC, data = finalData1)

OppMod <- lm(PostIPT ~  PreIPT + PositionC + OnWinningTeam + Conference + OppTeamC, data = finalData1)
anova(OppMod)
```
...
gf_boxplot(PostIPT ~ PositionC, data = finalData1)%>%
  gf_labs(title = "Post-Injury Play-Time vs. PositionC")
...
gf_boxplot(PostIPT ~ amtGamesMissedC, data = finalData1)%>%
  gf_labs(title = "Post-Injury Play-Time vs. amtGamesMissedC")
...
pred(PreIPT = 32, amtGamesMissed = 0)
