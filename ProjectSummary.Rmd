...
finalData <- filter(finalData, PreseasonInjury == "No") ## filters out players who have gotten preseason injury bc not enough observations. 
finalData <- filter(studydata, Season != "2014/2015") ## our sample data


finalData1 <- select(finalData, "PostIPT" , "amtGamesMissed", "PreIPT", "TotalSnaps", "WeekOfInjury", "AgeAtConcussion", "PreseasonInjury", "Position" )
```
...
##will remove Total snaps bc it is highly correlated with preInjury play time. 
finalData1 <- select(finalData, "PostIPT" , "PreIPT", "WeekOfInjury", "AgeAtConcussion", "Position")


fm.full <- lm(PostIPT ~ . , data = finalData1)
anova(fm.full)
...
mod2 <- lm(PostIPT ~ PreIPT + Position, data = finalDataAug, subset =  abs(finalDataAug$.resid) < 50 )## need to describe who they were and why you are removing (removed 1 person)
anova(mod2)
msummary(mod2)
mplot(mod2, which = 1 )
mplot(mod2, which = 2 )
