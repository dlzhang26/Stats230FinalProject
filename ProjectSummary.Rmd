...
studydata <- filter(studydata, PreseasonInjury == "No") ## filters out players who have gotten preseason injury (16) bc not enough observations. 


finalData <- filter(studydata, Season != "2014/2015") ## our sample data


finalData1 <- select(finalData, "PostIPT" , "amtGamesMissed", "PreIPT", "TotalSnaps", "WeekOfInjury", "AgeAtConcussion", "PreseasonInjury", "Position", "Conference", "Team", "OppTeamC", "OnWinningTeam")
```
...
##qualitative
m6 <- gf_bar(~ Position, data = finalData1, fill = c("lavender", "skyblue"))
...
OppMod <- lm(PostIPT ~  PreIPT + Position + OnWinningTeam + Conference + OppTeamC, data = finalData1)
anova(OppMod)
```
...
#ggpairs(finalData1)
finalData1 <- select(finalData, "Player","PostIPT" , "PreIPT", "WeekOfInjury", "AgeAtConcussion", "Position", "amtGamesMissed")


fm.full <- lm(PostIPT ~ PreIPT + amtGamesMissed +WeekOfInjury + AgeAtConcussion + Position , data = finalData1)
anova(fm.full)


modA <- lm(PostIPT ~ PreIPT + amtGamesMissed + Position, data = finalData1)
anova(modA)
msummary(modA)
mplot(modA, which = 1 )
mplot(modA, which = 2 )
...
mod1 <- lm(PostIPT ~ PreIPT + amtGamesMissed, data = finalData1)
anova(mod1)
msummary(mod1)
...
```{r}
full  <- lm(PostIPT ~ PreIPT + amtGamesMissed +WeekOfInjury + AgeAtConcussion + Position , data = FDfiltered)
anova(full)
three  <- lm(PostIPT ~ PreIPT + amtGamesMissed + Position , data = FDfiltered)
twoPred  <- lm(PostIPT ~ PreIPT + amtGamesMissed , data = FDfiltered)
