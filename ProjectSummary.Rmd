...
#ggpairs(finalData1)
finalData1 <- select(finalData, "Player","PostIPT" , "PreIPT", "WeekOfInjury", "AgeAtConcussion", "PositionC", "amtGamesMissed")


fm.full <- lm(PostIPT ~ PreIPT + amtGamesMissed +WeekOfInjury + AgeAtConcussion + PositionC , data = finalData1)
anova(fm.full)


modA <- lm(PostIPT ~ PreIPT + amtGamesMissed + PositionC, data = finalData1)
anova(modA)
msummary(modA)
mplot(modA, which = 1 )
mplot(modA, which = 2 )
...
```{r}
full  <- lm(PostIPT ~ PreIPT + amtGamesMissed +WeekOfInjury + AgeAtConcussion + PositionC , data = FDfiltered)
anova(full)
three  <- lm(PostIPT ~ PreIPT + amtGamesMissed + PositionC , data = FDfiltered)
